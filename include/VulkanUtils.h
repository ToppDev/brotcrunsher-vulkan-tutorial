...
#define ASSERT_VULKAN(val) if(val != VK_SUCCESS) DEBUG_BREAK;
...
void endSingleTimeCommandBuffer(VkDevice device, VkQueue queue, VkCommandPool commandPool, VkCommandBuffer commandBuffer)
{
    ...
    // Jede Graphics Queue, hat auch das Transfer bit (kann aber auch queue suchen mit nur transfer bit für mehr Geschwindigkeit)
    result = vkQueueSubmit(queue, 1, &submitInfo, VK_NULL_HANDLE);
    ASSERT_VULKAN(result);

    // schöner wäre auf Fence warten
...
void createImage(VkDevice device, VkPhysicalDevice physicalDevice, uint32_t width, uint32_t height, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usageFlags, VkMemoryPropertyFlags propertyFlags, VkImage &image, VkDeviceMemory &imageMemory)
{
    ...
    imageCreateInfo.format = format; //VK_FORMAT_R8G8B8A8_UNORM; // = Format used in STB Image
...
void createImageView(VkDevice device, VkImage image, VkFormat format, VkImageAspectFlags apspectFlags, VkImageView &imageView)
{
    ...
    imageViewCreateInfo.subresourceRange.aspectMask = apspectFlags;
